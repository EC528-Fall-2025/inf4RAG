...
#
# Usage:
#   ./run_and_process.sh <experiment_set_name> [path_to_sheet.csv]
#
# Arguments:
#   - experiment_set_name: (Required) The name for the set, which must match a key
#     in the processing script.
#     Examples: tensor-parallelism, pipeline-parallelism, pd-disaggregation, pd-disaggregation-nixl, pd-disaggregation-p2p
#
...
if [ "$EXPERIMENT_SET_NAME" = "pd-disaggregation" ] || [ "$EXPERIMENT_SET_NAME" = "pd-disaggregation-nixl" ] || [ "$EXPERIMENT_SET_NAME" = "pd-disaggregation-p2p" ]; then
  echo "Note: Using reduced intensity (256 prompts) for PD disaggregation variants"
  python3 "$BENCH_SCRIPT_PATH" \
    --host "$HOST" \
    --port "$PORT" \
    --model-type "$MODEL_TYPE" \
